...
import { fetchReactSolution } from '@/lib/content/react-solution';
...
export function ReactIDEClient({ question }: ReactIDEClientProps) {
  const { saveReactAttempt, saveReactComplete, saveReactSelfGrade, reactState } = useProgress();
  const existingProgress = reactState.questions[question.id];
  const isAlreadyAttempted =
    (existingProgress?.attempts?.length ?? 0) > 0 || Boolean(existingProgress?.completed);
...
function IDELayout({
  ...
  const [solutionFiles, setSolutionFiles] = useState<Record<string, string> | null>(
    question.solutionCode ?? null,
  );
  const userFilesRef = useRef<Record<string, string>>({});

  function writeFileMap(files: Record<string, string>) {
    sandpack.resetAllFiles();
    for (const [file, code] of Object.entries(files)) {
      sandpack.updateFile(`/${file.replace(/^\//, '')}`, code);
    }
  }

  function toggleMode(mode: 'problem' | 'solution', files?: Record<string, string>) {
    ...
    if (mode === 'solution') {
      ...
      const solution = files ?? solutionFiles;
      if (solution) writeFileMap(solution);
    } else {
      writeFileMap(userFilesRef.current);
    }
    setViewMode(mode);
  }

  async function handleViewSolutionClick() {
    let files = solutionFiles;
    if (!files) {
      files = await fetchReactSolution(question.id);
      setSolutionFiles(files);
    }
    if (files) toggleMode('solution', files);
    onViewSolution();
  }
