...
function stripSolution(question: ReactQuestion): ReactQuestion {
  const { solutionCode: _solutionCode, ...rest } = question;
  return rest;
}
...
export const getReactQuestions = cache((): ReactQuestion[] => {
  ...
  for (const entry of manifest.questions) {
    ...
    if (question) {
      questions.push(stripSolution(question));
    }
  }
...
export function getReactQuestion(id: string): ReactQuestion | null {
  const question = readJson<ReactQuestion>(`en/${id}.json`);
  return question ? stripSolution(question) : null;
}

/** Raw solution map from generated JSON. Do not call from SSG page props. */
export function getReactSolution(id: string): Record<string, string> | null {
  const question = readJson<ReactQuestion>(`en/${id}.json`);
  return question?.solutionCode ?? null;
}
