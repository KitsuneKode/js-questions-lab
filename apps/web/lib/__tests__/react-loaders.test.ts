...
describe('react-loaders', () => {
  ...
  it('each question has required fields', () => {
    ...
    for (const question of questions) {
      ...
      expect(question.solutionCode).toBeUndefined();
  ...
  it('getReactQuestion omits solutionCode from the public payload', () => {
    const question = getReactQuestion('react-counter');
    expect(question).not.toBeNull();
    expect(question?.id).toBe('react-counter');
    expect(question?.solutionCode).toBeUndefined();
    expect(getReactSolution('react-counter')).toEqual(expect.any(Object));
    expect(Object.keys(getReactSolution('react-counter') ?? {}).length).toBeGreaterThan(0);
  });
  ...
  it('render props mouse challenge keeps a full-height preview container', () => {
    ...
    expect(getReactSolution('react-render-props-mouse')?.['App.tsx']).toContain('min-h-screen');
  });
